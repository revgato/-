...
def calc_landmark_list(frame, hand_landmarks):
    ...
    landmark_list = []     # List of Keypoint
    ...
    for landmark in hand_landmarks.landmark:
        ...
        # Ignore z because it does not help increase accuracy of model
...
# This function prints the corresponding labels and recorded numbers
def show_notification(frame, labels, num_labeled):
    for idx, label in enumerate(labels):
        cv2.putText(frame, "{} (press {}). Recorded: {}".format(
            label, idx, num_labeled[idx]), 
            (10, 30 * (idx + 1)),
...
# Define data frame that will contain the corresponding 
# keypoint records and labels
df = pd.DataFrame()
...
# This array stores the number of records corresponding to the labels
    ...
    while True:
        ...
        if ret:
            ...
            if results.multi_hand_landmarks:
                ...
                # Labeling data by pressing number in keyboard

                key = cv2.waitKey(1)
                if key == 27:    # ESC
                    break

                elif 48 <= key < 48 + label_num:
                    label = key - 48

                    # Store the label into the array of keypoints
                    hand_landmarks_calc = np.append(label, hand_landmarks_calc)

                    # Append recorded keypoint to dataframe
                    df = df.append([hand_landmarks_calc], ignore_index = True)
                    num_labeled[label] += 1
        ...
        # Show the corresponding labels and recorded numbers
        frame = show_notification(frame, labels, num_labeled)
...
data_path = os.path.join(os.getcwd(), "model/keypoint_data.csv")
df.to_csv(data_path, mode = 'a', index = False, header = False)

print("Saved {} record to".format(sum(num_labeled)) + data_path)
